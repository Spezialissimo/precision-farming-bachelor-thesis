...
\note{Rifare qua la parte commentata}
% La \cref{introduzione-agricoltura-precisione} introduce il concetto di agricoltura di precisione come strumento per l'ottimizzazione delle risorse, la \cref{componenti-chiave} elenca le sue principali componenti, la \cref{irrigazione-di-precisione} illustra alcuni esempi di tecnologie utilizzate, mentre la \cref{limiti} copre i limiti attuali di queste tecnologie.
...
% \subsection{Formulazione di un sistema di irrigazione di precisione}
...
In \note{cita studio} viene presentato un sistema che fa utilizzo di entrambe le tipologie di sensori, ma tipicamente è un approccio non utilizzato a causa dell'elevato costo.
...
% COPIA E INCOLLA Gli autori di [19] sottolineano la necessità di condurre casi di studio pratici e sul campo piuttosto che affidarsi esclusivamente a ricerche condotte in ambienti simulati o controllati (ad esempio, serre), sostenendo che i risultati derivati da tali ambienti controllati spesso non sono rilevanti o trasferibili ad altri contesti oltre a quelli specificamente descritti negli studi.%
...
% \subsection{Approcci nel campo della teoria dei controlli}
